...
By default, you cannot create a `Product` with a `storeId` that does not exist. Also you cannot remove a `Store` if it has any `Products`.

<Callout>
  To allow remove a `Store` even if it has `Products`, you can set `skipRelationCheckOnRemove` to `true` in the options;
</Callout>
