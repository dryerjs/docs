...
const UserOnly = () =>
  applyDecorators(Role(UserRole.USER), UseGuards(RoleGuard));
const AdminOnly = () =>
  applyDecorators(Role(UserRole.ADMIN), UseGuards(RoleGuard));
...
```graphql copy
mutation RemoveUser {
  removeUser(id: "000000000000000000000000") {
    success
  }
}
```
...
`admin@dryerjs.com` can see this post, he is the creator of this post also an `ADMIN`.
`user@dryerjs.com` or `GUEST` cannot see this post because it is not public and they are not the creator of this post.
